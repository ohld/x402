...
12. **Facilitator** settles the payment: extracts the signed body from the internal message BoC, wraps it in a new internal message from its own wallet attaching TON for gas (estimated via emulation), signs and broadcasts. The user's W5 wallet verifies the signature and executes the Jetton transfer.
...
The payload contains only the signed settlement BoC and the asset identifier. All other fields (sender address, amount, destination, public key, forward payload, forward ton amount) are derived from the BoC by the facilitator:
...
- Simulation CAN replace the checks in sections 3 and 4, but the checks from sections 1 and 2 remain mandatory.
